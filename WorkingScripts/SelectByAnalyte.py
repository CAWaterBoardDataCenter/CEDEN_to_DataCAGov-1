...
def convert_txt_to_pipe(path, fileName, analytes, newFileName):
	name, ext = os.path.splitext(fileName)
	file = os.path.join(path, fileName)
	#newfile = name + '_filtered.txt'
		...
		with open(fileOut, 'w', newline='', encoding='utf8') as txtfileOut:
			...
			for row in reader:
				...
				for analyte in analytes:
					analyteList = analyteList + [rowDict["Analyte"] == analyte]
				if any(analyteList):
					writer.writerow(row)
...
if __name__ == "__main__":
	...
	convert_txt_to_pipe(path, fileName, analytes, newFileName)
